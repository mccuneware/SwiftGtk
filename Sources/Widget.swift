...
public class Widget {    
    ...
    private func removeSignals() {
        for (handlerId, _) in signals {
            disconnectSignal(instance:widgetPointer, handlerId: handlerId)
        }
    ...
    func addSignal(name: String, callback: SignalCallbackZero) {
        ...
        let handlerId = connectSignal(instance:widgetPointer, name: name, data: unsafeAddress(of:box), handler: unsafeBitCast(handler, to:GCallback.self))
